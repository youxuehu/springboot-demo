...
public class TFIDFAnalyzer
{
	...
	static HashMap<String,BigDecimal> idfMap;
	static HashSet<String> stopWordsSet;
	static BigDecimal idfMedian;
	...
	public List<Keyword> analyze(String content,int topN){
		...
		Map<String, BigDecimal> tfMap=getTF(content);
		for(String word:tfMap.keySet()) {
			// 若该词不在idf文档中，则使用平均的idf值(可能定期需要对新出现的网络词语进行纳入)
			if(idfMap.containsKey(word)) {
				keywordList.add(new Keyword(word,idfMap.get(word).multiply(tfMap.get(word))));
			}else
				keywordList.add(new Keyword(word,idfMedian.multiply(tfMap.get(word))));
		}
	 ...
	 * @return
	 */
	private Map<String, BigDecimal> getTF(String content) {
		Map<String,BigDecimal> tfMap=new HashMap<>();
		...
		for(String word:freqMap.keySet()) {
			tfMap.put(word,new BigDecimal(freqMap.get(word)).multiply(new BigDecimal(0.1)).divide(new BigDecimal(wordSum), 11, BigDecimal.ROUND_HALF_UP));
		}
	 ...
	 * @param in
	 */
	private void loadIDFMap(Map<String,BigDecimal> map, InputStream in ){
		BufferedReader bufr;
		try
		{
			bufr = new BufferedReader(new InputStreamReader(in));
			...
			while((line=bufr.readLine())!=null) {
				...
				map.put(kv[0],new BigDecimal(kv[1]));
			}
			try
			{
				bufr.close();
			...
			// 计算idf值的中位数
			List<BigDecimal> idfList=new ArrayList<>(map.values());

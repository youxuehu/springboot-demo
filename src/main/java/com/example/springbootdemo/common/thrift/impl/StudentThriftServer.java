...
@Service
public class StudentThriftServer implements InitializingBean {
    //监听的端口
    @Value("${server.thrift.port}")
    private Integer port;

    //线程池最小线程数
    @Value("${server.thrift.min.threadpool}")
    private Integer minThreadPool;

    //线程池最大线程数
    @Value("${server.thread.max.threadpool}")
    ...
    public void start() {
        try {
            ...
            //泛型就是实现的业务
            StudentService.Processor<StudentServiceImpl> processor = new StudentService.Processor<StudentServiceImpl>(studentServiceImpl);

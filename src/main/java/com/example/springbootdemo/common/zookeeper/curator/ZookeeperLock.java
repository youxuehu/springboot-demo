...
//    @Autowired
    ...
    public boolean tryLock() {
        try {
            ...
            LOG.info("当前线程" + Thread.currentThread().getName() + "初始化根节点 {}", lockPath);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            nodeCache.getListenable().addListener(new NodeCacheListener() {
                ...
                public void nodeChanged() throws Exception {
                    cdl.countDown();
                    LOG.info(beforePath + "节点监听事件触发，重新获得节点内容为：{}", new String(nodeCache.getCurrentData().getData()));
                }
            });
        } catch (Exception e) {
            e.printStackTrace();

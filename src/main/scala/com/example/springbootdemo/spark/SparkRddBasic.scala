package com.example.springbootdemo.spark
...
object SparkRddBasic extends App {
  ...
  val result = new SparkContext(new SparkConf().setAppName("wordcount").setMaster("local[2]"))
  ...
  result.foreach((i:String) => println(i))
}
